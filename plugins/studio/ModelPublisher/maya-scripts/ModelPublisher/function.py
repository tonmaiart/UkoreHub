...
into ModelPublisher's Repo Studio Setting tab (settings_page.py, UkoreHub
...
def publish(ticket: str):
    """Exports the current scene as FBX + a raw Maya Ascii copy into
    PublishApi's resolved publish root, versioned automatically (mirrors
    the original UkorePublisher's Model branch: fbx + ma, same as its
    "Model, Proxy, Hi > Fbx, ma" comment described). Returns
    ...
    fbx_path = os.path.join(version_dir, "{}_v{:03d}.fbx".format(ticket, version))
