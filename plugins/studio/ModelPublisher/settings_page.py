...
class ModelPublisherSettingsPage(QWidget):
    """Repo Studio Setting for ModelPublisher — lets a studio admin pick
    ...
    def refresh(self) -> None:
        ...
        if not self._outputs:
            self.hint_label.setText(
                ...
                "ModelPublisher has nothing to publish to. Add one under Repository "
        ...
        if len(self._outputs) == 1:
            self.hint_label.setText(
                "Only one pipeline connection declared — ModelPublisher uses it automatically, no choice needed."
            )
        else:
            self.hint_label.setText(
                "This repo has multiple pipeline connections declared — pick which one ModelPublisher publishes into."
            )
