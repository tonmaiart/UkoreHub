...
class AnimationPublisherSettingsPage(QWidget):
    """Repo Studio Setting for AnimationPublisher — lets a studio admin
    pick which of the active repo's declared pipeline connections
    ("Connect Pipeline Input Path...", each a specific CustomPath within a
    target repo, see plugins/studio/project_editor/pipeline_store.py) this
    tool should actually publish into. Replaces the free-text "Custom
    Path" field artists used to type directly in Maya, removed
    2026-07-19 — see plugins/studio/PublishApi/README.md's
    `get_publish_root(tool_id)` for how this choice is read back on the
    Maya side (falls back to "the repo's only declared connection"
    automatically if there's exactly one, so this tab only needs a real
    decision when a repo has more than one). Same
    ...
    def refresh(self) -> None:
        ...
        if not self._outputs:
            self.hint_label.setText(
                ...
                "AnimationPublisher has nothing to publish to. Add one under Repository "
        ...
        if len(self._outputs) == 1:
            self.hint_label.setText(
                "Only one pipeline connection declared — AnimationPublisher uses it automatically, no choice needed."
            )
        else:
            self.hint_label.setText(
                "This repo has multiple pipeline connections declared — pick which one AnimationPublisher publishes into."
            )
