...
VERSION = "1.0.0"
TYPE = "es_arm_01" #this name must match with your component folder ,
NAME = "arm"  # and Do not have any syntax.
DESCRIPTION = "Arm Rig"
...
class Guide(guide.ComponentGuide):
    ...
    def postInit(self):
        ...
        self.save_transform = ["root","low","end","eff"]

    def addObjects(self):
        """Add the Guide Root, blade and locators"""
        self.root = self.addRoot()
        self.root.attr("is_arm").set(True)

        self.loc_elbow = self.addLoc(
            name="low", parent=self.root, position=[3, 0, -0.1]
        )
        self.loc_wrist = self.addLoc(
            name="end", parent=self.loc_elbow, position=[6, 0, 0]
        )
        self.loc_eff = self.addLoc(
            name="eff", parent=self.loc_wrist, position=[7, 0, 0]
        )

        self.addDispCurve(
            "crv", [self.root, self.loc_elbow, self.loc_wrist, self.loc_eff]
        )
    ...
    def addParameters(self):
        ...
        self.addParam("fk_as_start", "bool", True,keyable=True)
