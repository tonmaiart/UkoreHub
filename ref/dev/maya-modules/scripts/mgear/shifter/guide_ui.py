...
#
# Created: Tue Dec 17 09:38:50 2024
...
class Ui_Form(object):
    ...
    def retranslateUi(self, Form):
        ...
        self.jointWorldOri_checkBox.setToolTip(gqt.fakeTranslate("Form", "<html><head/><body><p>Force all the joint to be oriented in World Space</p></body></html>", None, -1))
        self.jointWorldOri_checkBox.setText(gqt.fakeTranslate("Form", "Force World Oriented", None, -1))
        self.connect_joints_checkBox.setToolTip(gqt.fakeTranslate("Form", "<html><head/><body><p>When this option is active, mGear Shifter will try to connect to existing joints in the scene</p><p><span style=\" font-weight:600;\">WARNING</span>: The joints need to have the rotation values freeze to 0, 0, 0. If not we will connect using constrains instead of matrix connections </p><p>Use Maya\'s Modify &gt;&gt; Freeze Transformation command to Freeze rotation. Open the command options and ensure only Rotation is freeze, before execute the command</p><p>Freeze joint rotations steps:</p><p>1. Select all joints to freeze</p><p>2. Open Maya\'s Modify &gt;&gt; Freeze Transformation options and ensure only rotation is checked</p><p>3. Apply Modify &gt;&gt; Freeze Transformation</p><p><br/></p></body></html>", None, -1))
        self.connect_joints_checkBox.setText(gqt.fakeTranslate("Form", "Connect to existing joints.", None, -1))
