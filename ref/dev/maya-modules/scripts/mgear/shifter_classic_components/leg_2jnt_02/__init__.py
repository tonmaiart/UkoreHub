...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        po_vec = datatypes.Vector(0.5 * self.length0 * self.n_factor, 0, 0)
        ...
        po_vec = datatypes.Vector(0.5 * self.length1 * self.n_factor, 0, 0)
        ...
        po_vec = datatypes.Vector(0.5 * self.length2 * self.n_factor, 0, 0)
        ...
        self.ctrn_loc = primitive.addTransformFromPos(
            self.root_ctl, self.getName("ctrn_loc"), self.guide.apos[1]
        )
        ...
        # angle reader ----------------------------------------
        t = transform.getTransformLookingAt(
            self.guide.apos[1], self.guide.apos[0], self.binormal, "yz"
        )
        self.readerA = primitive.addTransform(
            self.root, self.getName("readerA_loc"), t
        )
    ...
    def addAttributes(self):
        ...
        self.knee_thickness_att = self.addAnimParam(
            "kneeAutoThickness",
            "Knee Auto Thickness",
            "double",
            self.settings["kneeThickness"],
            0,
            1,
        )
    ...
    def addOperators(self):
        ...
        add_node = node.createAddNode(self.roundness_att, 0.001)
        ...
        for i, div_cns in enumerate(self.div_cns):
            ...
            if self.negate:
                o_node = applyop.gear_rollsplinekine_op(
                    div_cns,
                    [self.tws2_rot, mid_twist, self.tws0_rot],
                    1.0 - perc,
                    subdiv,
                )
            else:
                o_node = applyop.gear_rollsplinekine_op(
                    div_cns,
                    [self.tws0_rot, mid_twist, self.tws2_rot],
                    perc,
                    subdiv,
                )
        ...
        node.createMulNode(
            div_thick_node.outputX, self.readerB.rx, self.thick_lvl.tx
        )
