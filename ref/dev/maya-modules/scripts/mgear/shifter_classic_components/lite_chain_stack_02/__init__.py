...
class Component(component.Main):
    ...
    def addAttributes(self):
        ...
        if self.settings["masterChainLocal"]:
            self.blend_local_att = self.addAnimParam("localBlend",
                                                     "Local Blend",
                                                     "double",
                                                     1,
                                                     0,
                                                     1)
        if self.settings["masterChainGlobal"]:
            self.blend_world_att = self.addAnimParam("worldBlend",
                                                     "World Blend",
                                                     "double",
                                                     1,
                                                     0,
    ...
    def connect_blend(self, mm_node, blend_attr, output):
        dm_node = node.createDecomposeMatrixNode(mm_node.output)
        pb_rt = node.createPairBlend(blender=blend_attr)
        pm.connectAttr(dm_node.outputTranslate, pb_rt.inTranslate2)
        pm.connectAttr(dm_node.outputRotate, pb_rt.inRotate2)

        pb_s = node.createPairBlend(blender=blend_attr)
        pm.connectAttr(dm_node.outputScale, pb_s.inTranslate2)

        # connec output
        pm.connectAttr(pb_rt.outTranslate,
                       output.attr("translate"), f=True)
        pm.connectAttr(pb_rt.outRotate,
                       output.attr("rotate"), f=True)
        pm.connectAttr(pb_s.outTranslate,
                       output.attr("scale"), f=True)
    ...
    def connect_master(self, mstr_out, slave_in, idx, offset, blend_attr):
        ...
        pb_rt = node.createPairBlend(inputB=m_out, blender=blend_attr)
        pm.connectAttr(pb_rt.outTranslate, s_in.translate)
        pm.connectAttr(pb_rt.outRotate, s_in.rotate)
        pb_s = node.createPairBlend(blender=blend_attr)
        pm.connectAttr(m_out.scale, pb_s.inTranslate2)
        pm.setAttr(pb_s.inTranslate1, [1, 1, 1])
        pm.connectAttr(pb_s.outTranslate, s_in.scale)
