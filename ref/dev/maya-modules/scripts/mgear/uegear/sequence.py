...
def get_sequence_node(get_only_first=True, as_sequence_node=True):
    ...
    for transform_node in transform_nodes:
        if not cmds.attributeQuery(
            "isSequence", node=transform_node, exists=True
        ):
            continue
...
def create_sequence_node_instance_from_maya_node(node):
    ...
    if not is_sequence:
        logger.warning(
            "Given Maya node: {} is not a sequence node!".format(node)
        )
        return None
    ...
    sequence_path = utils.clean_path(
        os.path.join(sequence_directory, sequence_name)
    )
...
def get_sequence_node_instance(
    ...
    if (
        len(sequence_name) < 3
        or not sequence_stage
        or sequence_stage not in SEQUENCE_STAGES
    ):
        logger.warning(
            "Sequence name length must be lower than 3 and sequence must defined a stage"
        )
        return None
    ...
    if (
        not is_scene_file
        and sequence_stage == ANIMATION_STAGE
        and shot_number == 0
    ):
        logger.warning(
            "Animation sequence shot file must define a shot number greater than 0"
        )
        return None
...
def create_sequence(
    ...
    scene_name="",
    notes="",
    delete_existing_sequences=True,
):
    """
...
def add_sequence_attributes(
    ...
    cmds.setAttr(
        "{}.sequenceDirectory".format(node), sequence_directory, type="string"
    )
...
def get_sequence_save_directory(
    sequence_directory, is_shot, shot_number, stage, scene_name
):
    """
    ...
    if is_shot:
        if shot_number != 0:
            ...
            shot_directory = os.path.join(
                shots_directory, "{0:0=3d}".format(shot_number)
            )
...
def update_sequence_node_version(node, version):
    ...
    if (
        ...
        or not cmds.attributeQuery("versionNumber", node=node, exists=True)
    ):
        return False
...
def save_sequence_file(
    directory, sequence_name, stage, version, is_shot, shot_number, scene_name
):
    if not is_shot:
        save_as = "{}_{}_{}_v{}".format(
            sequence_name, scene_name, stage, "{0:0=3d}".format(version)
        )
    else:
        if shot_number == 0:
            save_as = "{}_{}_v{}".format(
                sequence_name, stage, "{0:0=3d}".format(version)
            )
        else:
            save_as = "{}_shot_{}_{}_v{}".format(
                sequence_name,
                "{0:0=3d}".format(shot_number),
                stage,
                "{0:0=3d}".format(version),
            )
...
def save_new_sequence_version(
    ...
    save_directory = get_sequence_save_directory(
        sequence_directory=sequence_path,
        is_shot=is_shot,
        shot_number=shot_number,
        stage=sequence_stage,
        scene_name=scene_name,
...
def handle_shot_namespace_conflict(name):
    ...
    name_number = 1 + len(
        [
            x
            for x in map(
                str, cmds.namespaceInfo(listOnlyNamespaces=True, recurse=True)
            )
            if "tempName" in x
        ]
    )
    ...
    associated_shots = [
        x
        for x in cmds.ls(type="shot")
        if name in cmds.shot(x, q=True, cc=True)
    ]
    if associated_shots:
...
def get_shot_sequence_start_time():
    ...
    last_shot_end = (
        int(max([cmds.shot(s, set=True, q=True) for s in shots]))
        if shots
        else 0
    )
...
def get_new_shot_number():
    cam_names = [
        c[-3:]
        for c in cmds.namespaceInfo(listOnlyNamespaces=True)
        if "shot" in c
    ]
    ...
    if cam_names:
        ...
        latest_camera_number = 10 + (
            math.floor(int(latest_camera[-3:]) / 10) * 10
        )
...
def get_shot_reference_camera(sequence_name, camera_path=None, shot_number=0):
    ...
    if camera_path and os.path.isfile(camera_path):
        camera_reference = cmds.file(
            camera_path, r=True, type="mayaAscii", ns=camera_namespace
        )
        cam = [
            c
            for c in cmds.referenceQuery(camera_reference, nodes=True)
            if cmds.objectType(c, isType="transform")
        ][0]
    else:
...
def create_sequence_shot(camera_path, sequence_node=None):
    ...
    if sequence_node.is_shot:
        ...
        camera = get_shot_reference_camera(
            camera_path, sequence_node.name, shot_number
        )
        shot = UeGearShot(
            ...
            camera=camera,
            stage=sequence_node.stage,
            version=sequence_node.version,
        )
        shot.create()
    else:
...
class UeGearSequenceNode(object):
    ...
    def path(self):
        ...
        return self._get_sequence_attribute(
            "sequenceDirectory", default=self._is_shot
        )
    ...
    def is_shot(self):
        ...
        return self._get_sequence_attribute(
            "isShotFile", default=self._is_shot
        )
    ...
    def version(self):
        ...
        return self._get_sequence_attribute(
            "versionNumber", default=self._version
        )
    ...
    def scene_name(self):
        ...
        return self._get_sequence_attribute(
            "sceneName", default=self._scene_name
        )
    ...
    def shot_number(self):
        ...
        return self._get_sequence_attribute(
            "shotNumber", default=self._shot_number
        )
    ...
    def _get_sequence_attribute(self, attribute_name, default=None):
        ...
        if (
            self._node
            and cmds.objExists(self._node)
            and cmds.attributeQuery(attribute_name, node=self._node, ex=True)
        ):
...
class UeGearShot(object):
    ...
    def __init__(
        ...
        self._number = number
        self._name = "{}_shot_{:0>3}".format(
            self._sequence_node.name, self._number
        )
        self._camera = camera
        self._start = start
        self._end = end
