...
def exportable_assets(
    tag_name=tag.TAG_ASSET_TYPE_ATTR_NAME, nodes=None, asset_type=None
):
    """
    ...
    tagged_node_attributes = tag.find_tagged_node_attributes(
        tag_name=tag_name, nodes=nodes
    )
...
def export_assets(
    tag_name=tag.TAG_ASSET_TYPE_ATTR_NAME, nodes=None, check_assets=True
):
    """
    ...
    if static_meshes:
        export_static_meshes(
            tag_name=tag_name, nodes=static_meshes, check_assets=False
        )
    ...
    if skeletons:
        export_skeletons(
            tag_name=tag_name, nodes=skeletons, check_assets=False
        )
...
def export_selected_assets(
    tag_name=tag.TAG_ASSET_TYPE_ATTR_NAME, check_assets=True
):
    """
    ...
    return export_assets(
        tag_name=tag_name, nodes=cmds.ls(sl=True), check_assets=check_assets
    )
...
def export_assets_json(
    ...
    **kwargs
    ...
    output_file_path = utils.clean_path(
        "{}.json".format(os.path.join(file_path, output_file))
    )
...
def export_static_meshes(
    ...
    static_meshes = utils.force_list(
        nodes
        or exportable_assets(tag_name, asset_type=tag.TagTypes.StaticMesh).get(
            tag.TagTypes.StaticMesh, list()
        )
    )
    ...
    output_path = utils.clean_path(
        os.path.join(os.path.dirname(maya_file_path), "output")
    )
    ...
    if not os.path.isdir(output_path):
        logger.error(
            'Was not possible to create export output directory: "{}"'.format(
                output_path
            )
        )
        return False
    ...
    group_name = (
        utils.get_first_in_list(cmds.listRelatives(nodes[0], p=True)) or ""
    )
    category = (
        utils.get_first_in_list(cmds.listRelatives(group_name, p=True)) or ""
    )
    if ":" in group_name:
        group_name = group_name.split(":")[-1]

    export_assets_json(
        source_file=maya_file_path,
        ...
        tag_name=tag.TagTypes.StaticMesh,
        nodes=nodes,
...
def export_alembics(
    ...
    alembics = utils.force_list(
        nodes
        or exportable_assets(tag_name, asset_type=tag.TagTypes.Alembic).get(
            tag.TagTypes.Alembic, list()
        )
    )
    if not alembics:
    ...
    version_string = maya_file_name.split("_")[-1].split(".")[0]
    scene_string = (
        "" if "assets" in maya_file_path else maya_file_path.split("/")[-3]
    )
    shot_string = (
        "" if "assets" in maya_file_path else maya_file_path.split("/")[-2]
    )
    ...
    for alembic in alembics:
        ...
        output_file = (
            ...
            + shot_string
            + "_"
            + "{}.abc".format(alembic.split(":")[0])
        )
        alembic_export_command = (
            "-frameRange "
            + str(start_frame)
            + " "
            + str(end_frame)
            + " -uvWrite -worldSpace -attr material -dataFormat ogawa -writeFaceSets -stripNamespaces "
            + root
            + " -file "
            + output_path
            + output_file
        ...
        logger.info(
            "Alembic Export command: {}".format(alembic_export_command)
        )
        ...
        export_assets_json(
            source_file=maya_file_path,
            file_path=output_path,
            output_file=output_file.replace(".abc", ""),
            category="",
            file_type="abc",
            group_name=alembic,
            is_shot=is_shot,
...
def export_skeletons(
    ...
    skeletons = utils.force_list(
        nodes
        or exportable_assets(tag_name, asset_type=tag.TagTypes.Skeleton).get(
            tag.TagTypes.Skeleton, list()
        )
    )
    if not skeletons:
    ...
    for skeleton in skeletons:
        ...
        for skin_cluster in skin_cluster_nodes:
            mesh_nodes = mesh_nodes + utils.get_skin_cluster_meshes(
                skin_cluster
            )
        if not mesh_nodes:
            logger.warning(
                "No skinned meshes found for given skeleton joint: {}".format(
                    skeleton
                )
            )
            continue
        ...
        namespace_str = (
            skeleton.split(":")[0] if ":" in skeleton else group_name
        )

        if is_shot:
            output_file = (
                scene_string + shot_string + "_" + namespace_str + ".fbx"
            )
        ...
        export_assets_json(
            source_file=maya_file_path,
            file_path=output_path,
            output_file=output_file.replace(".fbx", ""),
            category=category,
            file_type="fbx",
            group_name=group_name,
            is_shot=is_shot,
            ...
            shot_string=shot_string,
            scene_string=scene_string,
            tag_name=tag.TagTypes.Skeleton,
            nodes=[skeleton],
        ...
        if previous_parent and not cmds.referenceQuery(
            skeleton, isNodeReferenced=True
        ):
...
def export_cameras(
    ...
    cameras = utils.force_list(
        nodes
        or exportable_assets(tag_name, asset_type=tag.TagTypes.Camera).get(
            tag.TagTypes.Camera, list()
        )
    )
    if not cameras:
    ...
    for camera in cameras:
        camera_shape = utils.get_first_in_list(
            cmds.listRelatives(camera, shapes=True)
        )
        if not camera_shape:
            logger.warning(
                "Impossible to export camera: {} because it has no shapes"
            )
            continue
        ...
        custom_params = {
            ...
            "clippingPlanes": cmds.camera(
                camera_shape, q=True, clippingPlanes=True
            ),
            "farClipPlane": cmds.camera(
                camera_shape, q=True, farClipPlane=True
            ),
            "nearClipPlane": cmds.camera(
                camera_shape, q=True, nearClipPlane=True
            ),
            "focalLength": cmds.camera(camera_shape, q=True, focalLength=True),
            "fStop": cmds.camera(camera_shape, q=True, fStop=True),
            "horizontalFilmAperture": cmds.camera(
                camera_shape, q=True, horizontalFilmAperture=True
            )
            * 25.4,
            "verticalFilmAperture": cmds.camera(
                camera_shape, q=True, verticalFilmAperture=True
            )
            * 25.4,
            "lensSqueezeRatio": cmds.camera(
                camera_shape, q=True, lensSqueezeRatio=True
            ),
        }
        export_assets_json(
            source_file=maya_file_path,
...
def export_layout_json(nodes=None, output_path=None):
    ...
    output_file = utils.clean_path(
        os.path.join(
            output_path,
            "{}_layout.json".format(os.path.splitext(scene_file)[0]),
        )
    )
    ...
    if not os.path.isdir(output_path):
        logger.error(
            "Export directory does not exists: {}".format(output_path)
        )
        return False
    ...
    for mesh in found_meshes:
        ...
        (
            ...
            rotation_value,
            scale_value,
        ) = utils.get_transforms_for_mesh_node(mesh)
        source_name_output = short_mesh.rsplit("_", 1)[0]
        if ":" in source_name_output:
            source_name_output = source_name_output.split(":")[-1]
        asset_name = tag.tag_values(
            tag_name=tag.TAG_ASSET_NAME_ATTR_NAME, nodes=[mesh]
        )
        asset_path = tag.tag_values(
            tag_name=tag.TAG_ASSET_PATH_ATTR_NAME, nodes=[mesh]
        )
        actor_name = tag.tag_values(
            tag_name=tag.TAG_ACTOR_NAME_ATTR_NAME, nodes=[mesh]
        )
    ...
    if not result:
        ...
        logger.info(
            "Layout file exported successfully: {}".format(output_file)
        )
...
def check_scene(tag_name=tag.TAG_ASSET_TYPE_ATTR_NAME, nodes=None):
    ...
    if "/assets/" not in maya_file_path and "/scenes/" not in maya_file_path:
        logger.error(
            "Please place this Maya file under either and assets or scenes folder"
        )
        return False

    tagged_nodes_attributes_list = tag.find_tagged_node_attributes(
        tag_name=tag_name, nodes=nodes
    )
    ...
    for tagged_node_attribute in tagged_nodes_attributes_list:
        tagged_node = tagged_node_attribute.replace(".{}".format(tag_name), "")
        objects_map[str(cmds.getAttr(tagged_node_attribute))].append(
            tagged_node
        )
    ...
    for static_mesh in objects_map.get(tag.TagTypes.StaticMesh, list()):
        ...
        if instances:
            logger.warning(
                ...
                "be fixed and can lead to unexpected transform offset within Unreal Engine.".format(
                    static_mesh
                )
            )
            continue
        ...
        first_shape = (
            utils.get_first_in_list(
                cmds.listRelatives(static_mesh, shapes=True)
            )
            or None
        )
        if not first_shape or not cmds.objectType(first_shape) == "mesh":
            logger.error(
                'Static Mesh nodes must be mesh nodes. Please fix: "{}"'.format(
                    static_mesh
                )
            )
            continue
        ...
        if "/assets/" in maya_file_path:
            ...
            if (
                not translation == [0.0, 0.0, 0.0]
                or not rotation == [0.0, 0.0, 0.0]
                or not scale == [1.0, 1.0, 1.0]
            ):
                logger.error(
                    'Static Meshes nodes must have zero transforms. Please zero out: "{}"'.format(
                        static_mesh
                    )
                )
                continue
        ...
        asset_name_group = utils.get_first_in_list(
            cmds.listRelatives(static_mesh, p=True) or list()
        )
        if (
            asset_name_group
            and cmds.objectType(asset_name_group) == "transform"
        ):
            category_name_group = utils.get_first_in_list(
                cmds.listRelatives(asset_name_group, p=True) or list()
            )
        if not asset_name_group or not category_name_group:
            logger.error(
                ...
                'env/warehouse/<static_mesh>. Please fix: "{}"'.format(
                    static_mesh
                )
            )
            continue
    ...
    for joint in objects_map.get(tag.TagTypes.Skeleton, list()):
        if not cmds.objectType(joint) == "joint":
            logger.error(
                'Skeleton tags must be on a joint. Please fix: "{}"'.format(
                    joint
                )
            )
            continue
        ...
        for skeletal_mesh in mesh_list:
            category_name_group = None
            asset_name_group = utils.get_first_in_list(
                cmds.listRelatives(skeletal_mesh, p=True) or list()
            )
            if (
                asset_name_group
                and cmds.objectType(asset_name_group) == "transform"
            ...
            if not asset_name_group or not category_name_group:
                logger.error(
                    ...
                    'them, for example: env/warehouse/<static_mesh>. Please fix: "{}"'.format(
                        skeletal_mesh
                    )
                )
                continue
    ...
    for camera in objects_map.get(tag.TagTypes.Camera, list()):
        first_shape = (
            utils.get_first_in_list(cmds.listRelatives(camera, shapes=True))
            or None
        )
        if not first_shape or not cmds.objectType(first_shape) == "camera":
            logger.error(
                'Camera tags should be on the actual camera object. Please fix: "{}"'.format(
                    camera
                )
            )
            continue
...
def check_skeleton_type(root_joint):
    ...
    skeleton_definition_path = utils.join_path(
    ...
    if os.path.exists(skeleton_definition_path):
        ...
        for skeleton_definition_file in skeleton_definition_files:
            if exit_loop:
                break
            json_data = utils.read_json_file(
                utils.join_path(
                    skeleton_definition_path, skeleton_definition_file
                )
            )
    ...
    return (
        skeleton_definition_file.replace(".json", "")
        if skeleton_math
        else "custom"
    )
            ...
            if cmds.objExists(node):
                ...
                if shapes:
                    ...
                    if object_type == "mesh":
                        ...
                        parent = utils.get_first_in_list(
                            cmds.listRelatives(
                        ...
                        current_locator = cmds.spaceLocator(
                            name=node.replace("|", "_") + "_loc"
                        )[0]
                        cmds.matchTransform(current_locator, node)

                        current_offset_locator = cmds.spaceLocator(
                            name=current_locator.replace("_loc", "_offset_loc")
                        )[0]
                        cmds.matchTransform(current_offset_locator, node)
                        cmds.parent(node, current_locator)
                        cmds.matchTransform(
                            current_locator, SETUP_PIVOT_LOC_NAME
                        )
                        cmds.parent(node, world=True)
                        cmds.makeIdentity(
                            node, apply=True, t=True, r=True, s=True
                        )
                        cmds.parent(node, current_locator)

                        if not move_to_origin:
                            cmds.matchTransform(
                                current_locator, current_offset_locator
                            )
