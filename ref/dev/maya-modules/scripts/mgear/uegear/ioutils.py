...
def export_static_mesh(export_path, static_mesh, file_name=None):
    ...
    if not file_name:
        output_file = "{}.fbx".format(
            static_mesh.replace(":", "_").split("|")[-1]
        )
    ...
    if os.path.isfile(final_export_path):
        ...
        export_path = utils.join_path(
            os.path.dirname(final_export_path),
            "{}_uegear_temp{}".format(_file_name, _file_extension),
        )
    ...
    try:
        ...
        logger.error(
            'Something went wrong while exporting FBX in following path: "{}" | {}'.format(
                export_path, exc
            )
        )
        valid_export = False
    ...
    if export_path != final_export_path:
        ...
        if not os.path.isfile(final_export_path):
            logger.error(
                'Something went wrong while renaming exported FBX file from "{}" to "{}"'.format(
                    export_path, final_export_path
                )
            )
            return ""
