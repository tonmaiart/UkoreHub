...
class Component(component.Main):
    ...
    def addOperators(self):
        ...
        if self.isFk:
            ...
            for i,off in enumerate(self.fk_off):
                ctrl = self.fk_ctl[i]

                attribute.lockAttribute(ctrl,["sx","sy","sz"])

                if i == 0:
                    continue
                else:
                    off.setParent(self.fk_ctl[i-1])
        ...
        if self.settings["mode"] == 0:  # fk only
            ...
            pm.parentConstraint(self.fk_ctl[-1], self.loc[-1], maintainOffset=False)
