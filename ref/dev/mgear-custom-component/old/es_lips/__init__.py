...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        self.slide_plane = "target_plane"
        self.ignore_extra_joints = self.settings["ignore_extra_joints"]
        ...
        self.dict_main = {"upper_control":[],"lower_control":[],"upper_joint":[],"lower_joint":[],"upper_name":["L_main","up_main","R_main"],"lower_name":["low_main"]}
        self.dict_mid = {"upper_control":[],"lower_control":[],"upper_joint":[],"lower_joint":[],"upper_name":["R_up_corner","R_up","C_up","L_up","L_up_corner"],"lower_name":["R_low","C_low","L_low"]}
        self.dict_sub = {
        ...
        self.dict_piv = {"upper_matrix":[],"lower_matrix":[]}
        ...
        if self.ignore_extra_joints:
            self.list_part = ["01"]

        else:
            self.list_part = ["01", "02", "03"]

        def create_bind_joints():
            # create base joint
            self.jnt_pos.append({
                "obj": primitive.addTransform(self.grp_bind_loc, self.getName("base_loc"), m=transform.getTransform(self.root)),
                "name": "base",
            ...
            for value in self.list_part:
                list_name = self.dict_sub[value]["upper_name"]+self.dict_sub[value]["lower_name"]

                # create locator
                for name in list_name:
                    m_pos = transform.getTransformFromPos(self.guide.pos[name])

                    if name in self.dict_sub[value]["upper_name"]:
                        self.dict_sub[value]["upper_joint"].append(
                            primitive.addTransform(self.grp_bind_loc, self.getName("{}_loc".format(name)), m=m_pos)
                        )
                    elif name in self.dict_sub[value]["lower_name"]:
                        self.dict_sub[value]["lower_joint"].append(
                            primitive.addTransform(self.grp_bind_loc, self.getName("{}_loc".format(name)), m=m_pos)
                        )

                # create joint
                for i,locator in enumerate(self.dict_sub[value]["upper_joint"]+self.dict_sub[value]["lower_joint"]):
                    self.jnt_pos.append({
                        "obj": locator,
                        "name": list_name[i],
                        "newActiveJnt": False,
        ...
        def create_curve():
            # get position
            list_upper_pos = []
            for name in self.dict_sub["01"]["upper_name"]:
                list_upper_pos.append(self.guide.pos[name])

            list_lower_pos = []
            for name in self.dict_sub["01"]["lower_name"]:
            ...
            # duplicate
            # self.curve_lip_mid = pm.duplicate(self.curve_lip_main,n=self.getName("mid_crv"))[0]
        ...
        def create_main_joint():
            ...
            # snap position L,R
            pm.xform(self.L_main_joint,ws=1,m=transform.getTransform(self.dict_sub["01"]["upper_joint"][-1]))
            pm.xform(self.R_main_joint,ws=1,m=transform.getTransform(self.dict_sub["01"]["upper_joint"][0]))

            # snap position Up,Low
            for list_data in [["upper_joint",self.up_main_joint],["lower_joint",self.low_main_joint]]:
                part,joint = list_data

                if len(self.dict_sub["01"][part])%2 == 0:
                    loc = pm.spaceLocator()
                    i = int(len(self.dict_sub[part])/2)
                    pm.pointConstraint(self.dict_sub["01"][part][i],self.dict_sub["01"][part][i-1],loc)


                    m_pos_up = transform.getTransform(loc)
                    pm.delete(loc)
                else:
                    i = math.floor(len(self.dict_sub["01"][part])/2)
                    m_pos_up = transform.getTransform(self.dict_sub["01"][part][i])

                pm.xform(joint,ws=1,m=m_pos_up)
            ...
            # add npo
            # mgear.rigbits.addNPO(self.dict_main["lower_joint"]+self.dict_main["upper_joint"])

        def create_main_control():
            list_name = ["L_main","R_main","up_main","low_main"]
            for i, joint in enumerate([self.L_main_joint,self.R_main_joint,self.up_main_joint,self.low_main_joint]):
                control = self.addCtl(self.grp_control,
                                      list_name[i],
                ...
                # connect world space
                pm.connectAttr("{}.worldMatrix[0]".format(control),"{}.offsetParentMatrix".format(joint))
                attribute.reset_SRT([joint])

                mgear.rigbits.addNPO([control])

                # if joint in self.list_locator_upper:
                #     self.list_control_upper.append(control)
                # elif joint in self.list_locator_lower:
                #     self.list_control_lower.append(control)

        def create_sub_control():
            list_name = self.dict_sub["01"]["upper_name"]+self.dict_sub["01"]["lower_name"]

            for i,locator in enumerate(self.dict_sub["01"]["upper_joint"]+self.dict_sub["01"]["lower_joint"]):

                control = self.addCtl(self.grp_control,
                            list_name[i],
                            m=transform.getTransformFromPos(self.guide.pos[list_name[i]]),
                            color=4,
                            iconShape="cube",
                              w=0.1 * self.size,
                              h=0.1 * self.size,
                ...
                if locator in self.dict_sub["01"]["upper_joint"]:
                    self.dict_sub["01"]["upper_control"].append(control)
                elif locator in self.dict_sub["01"]["lower_joint"]:
                    self.dict_sub["01"]["lower_control"].append(control)

            # add npo
            mgear.rigbits.addNPO(self.dict_sub["01"]["upper_control"]+self.dict_sub["01"]["lower_control"])

        def create_mid_control():
            def snap_to_closest_point_on_curve(obj, curve):
                """
            ...
            for i in range(5):
                control = self.addCtl(self.grp_control_mid,
                                      self.getName("upper_mid_{}_ctrl".format(i+1)),
                                      m=transform.getTransform(self.grp_control_mid),
                                      color=9,
                                      iconShape="cube",
                                      w=0.15 * self.size,
                                      h=0.15 * self.size,
                ...
                if i == 0:
                    pm.matchTransform(control,self.list_control_upper[0],pos=1)
                elif i == 1:
                    constraint = pm.pointConstraint(self.list_control_upper[0],self.up_main_joint,control)
                    pm.delete(constraint)
                elif i == 2:
                    pm.matchTransform(control,self.up_main_joint,pos=1)
                elif i == 3:
                    constraint = pm.pointConstraint(self.up_main_joint,self.list_control_upper[-1],control)
                    pm.delete(constraint)
                elif i == 4:
                    pm.matchTransform(control,self.list_control_upper[-1],pos=1)

                snap_to_closest_point_on_curve(str(control),str(self.curve_lip_main))

                self.dict_mid["upper_control"].append(control)

            for i in range(3):
                control = self.addCtl(self.grp_control_mid,
                                      self.getName("lower_mid_{}_ctrl".format(i+1)),
                                      m=transform.getTransform(self.grp_control_mid),
                                      color=9,
                                      iconShape="cube",
                                      w=0.15 * self.size,
                ...
                if i == 0:
                    constraint = pm.pointConstraint(self.list_control_upper[0],self.low_main_joint,control)
                    pm.delete(constraint)
                elif i == 1:
                    pm.matchTransform(control,self.low_main_joint,pos=1)
                elif i == 2:
                    constraint = pm.pointConstraint(self.list_control_upper[-1],self.low_main_joint,control)
                    pm.delete(constraint)

                snap_to_closest_point_on_curve(str(control),str(self.curve_lip_main))

                self.dict_mid["lower_control"].append(control)

            # # connect world space
            # pm.connectAttr("{}.worldMatrix[0]".format(control),"{}.offsetParentMatrix".format(joint))
            # attribute.reset_SRT([joint])
            #
            mgear.rigbits.addNPO(self.dict_mid["lower_control"]+self.dict_mid["upper_control"])

        def create_mid_joint():
            def snap_to_closest_point_on_curve(obj, curve):
                """
                Snaps an object to the closest point on a curve using Maya API 2.0.
            ...
            for i in range(5):
                pm.select(cl=1)
                joint = pm.joint(n=self.dict_mid["upper_name"][i]+"_jnt")

                if i == 0:
                    pm.matchTransform(joint,self.dict_main["upper_joint"][-1],pos=1)
                elif i == 1:
                    constraint = pm.pointConstraint(self.dict_main["upper_joint"][-1],self.dict_main["upper_joint"][1],joint)
                    pm.delete(constraint)
                elif i == 2:
                    pm.matchTransform(joint,self.dict_main["upper_joint"][1],pos=1)
                elif i == 3:
                    constraint = pm.pointConstraint(self.dict_main["upper_joint"][0],self.dict_main["upper_joint"][1],joint)
                    pm.delete(constraint)
                elif i == 4:
                    pm.matchTransform(joint,self.dict_main["upper_joint"][0],pos=1)

                snap_to_closest_point_on_curve(str(joint),str(self.curve_lip_main))

                self.dict_mid["upper_joint"].append(joint)


            for i in range(3):
                pm.select(cl=1)
                joint = pm.joint(n=self.dict_mid["lower_name"][i]+"_jnt")

                if i == 0:
                    constraint = pm.pointConstraint(self.dict_main["upper_joint"][-1],self.dict_main["lower_joint"][0],joint)
                    pm.delete(constraint)
                elif i == 1:
                    pm.matchTransform(joint,self.dict_main["lower_joint"][0],pos=1)
                elif i == 2:
                    constraint = pm.pointConstraint(self.dict_main["upper_joint"][0],self.dict_main["lower_joint"][0],joint)
                    pm.delete(constraint)

                snap_to_closest_point_on_curve(str(joint),str(self.curve_lip_main))

                self.dict_mid["lower_joint"].append(joint)

            pm.parent(self.dict_mid["lower_joint"]+self.dict_mid["upper_joint"],self.grp_joint_mid)
            mgear.rigbits.addNPO(self.dict_mid["lower_joint"]+self.dict_mid["upper_joint"])
        ...
        def create_control_set():
            def create_sub_control():
                ...
                for i in range(len(self.dict_sub["01"]["upper_name"])):
                    control = self.addCtl(self.ctrl_big,
                                          self.getName(self.dict_sub["01"]["upper_name"][i]),
                ...
                for i in range(len(self.dict_sub["01"]["lower_name"])):
                    control = self.addCtl(self.ctrl_big,
                                          self.getName(self.dict_sub["01"]["lower_name"][i]),
                                          m=transform.getTransformFromPos(list_pos[i]),
                                          color=9,
                                          iconShape="cube",
                                          w=0.8 * self.size,
            ...
            # add npo
            mgear.rigbits.addNPO([self.ctrl_big]+self.dict_main["upper_control"]+self.dict_main["lower_control"]+self.dict_sub["01"]["upper_control"]+self.dict_sub["01"]["lower_control"])

            # parent sub control to main control
            for i in range(4):
                if i == 0: # L
                    list_control = self.dict_sub["01"]["upper_control"][4:]+self.dict_sub["01"]["lower_control"][3:]
                    list_control_npo = [control.getParent() for control in list_control]
                    ctrl_main = self.dict_main["upper_control"][0]
                    name = "L"

                elif i == 1: # R
                    list_control = self.dict_sub["01"]["upper_control"][0:3] + self.dict_sub["01"]["lower_control"][0:2]
                    list_control_npo = [control.getParent() for control in list_control]
                    ctrl_main = self.dict_main["upper_control"][2]
                    name = "R"

                elif i == 2: # UP
                    list_control = [self.dict_sub["01"]["upper_control"][3]]
                    list_control_npo = [control.getParent() for control in list_control]
                    ctrl_main = self.dict_main["upper_control"][1]
                    name = "up"

                elif i == 3:  # LOW
                    list_control = [self.dict_sub["01"]["lower_control"][2]]
                    list_control_npo = [control.getParent() for control in list_control]
                    ctrl_main = self.dict_main["lower_control"][0]
                    name = "low"

                print("list npo :",list_control_npo)
        ...
        def get_matrix_pivot():
            # create locator
            for key in self.guide.tra.keys():
                if "up_piv" in key:
                    self.dict_piv["upper_matrix"].append(self.guide.tra[key])
                elif "low_piv" in key:
                    self.dict_piv["lower_matrix"].append(self.guide.tra[key])
        ...
        def create_jaw():
            ...
            self.loc_jaw = primitive.addTransform(self.grp_bind_loc, self.getName("jaw_loc"), m=transform.setMatrixScale(self.guide.tra["jaw"],(1,1,1)))
    ...
    def addOperators(self):
        ...
        def create_uv_pin_surface_node():
            ...
            pm.connectAttr("{}.worldSpace[0]".format("target_plane"),"{}.deformedGeometry".format(self.node_uvPin_surface))
        ...
        def create_sliding_locator():
            ...
            for index,part in enumerate(self.list_part):
                ...
                for i,name in enumerate(self.dict_sub[part]["upper_name"]+self.dict_sub[part]["lower_name"]):
                    control = (self.dict_sub["01"]["upper_control"] + self.dict_sub["01"]["lower_control"])[i]
                    ...
                    if part == "01": # connect pin position directly
                        ...
                        list_01_loc = self.dict_sub["01"]["upper_pin"] + self.dict_sub["01"]["lower_pin"]
                    ...
                    if part == "01":
                        ...
                        list_loc_bind = self.dict_sub["01"]["upper_joint"]+self.dict_sub["01"]["lower_joint"]
                        if name in self.dict_sub["01"]["upper_name"]:
                        ...
                        elif name in self.dict_sub["01"]["lower_name"]:
        ...
        def create_pinch_control():
            ...
            for control in self.dict_main["upper_control"]+self.dict_main["lower_control"]:
                if control == self.dict_main["lower_control"][0]:
                    keyword = "LOW"
                    list_target = ((self.dict_sub["01"]["lower_control"][1],-0.025),
                                   (self.dict_sub["01"]["lower_control"][3], 0.025))

                elif control == self.dict_main["upper_control"][1]:
                    keyword = "UP"
                    list_target = ((self.dict_sub["01"]["upper_control"][2],-0.025),
                                   (self.dict_sub["01"]["upper_control"][4], 0.025))

                elif control == self.dict_main["upper_control"][0]:
                    keyword = "L"
                    list_target = ((self.dict_sub["01"]["upper_control"][5], -0.01),
                                   (self.dict_sub["01"]["lower_control"][4], -0.01))

                elif control == self.dict_main["upper_control"][2]:
                    keyword = "R"
                    list_target = ((self.dict_sub["01"]["upper_control"][1], 0.01),
                                   (self.dict_sub["01"]["lower_control"][0], 0.01))
                else:
                    continue
            ...
            for control in self.dict_main["upper_control"]+self.dict_main["lower_control"]:
                if control == self.dict_main["upper_control"][0]:
                    keyword = "L"
                    dict_target = {
                        self.dict_sub["01"]["upper_control"][5]:(("up-out",-0.5),("up",-1),("down-out",0.5),("down",1)),
                        self.dict_sub["01"]["lower_control"][4]:(("up-out",-0.5),("up",-1),("down-out",0.5),("down",1))
                        }
                    ...
                    keyword = "R"
                    dict_target = {
                        self.dict_sub["01"]["upper_control"][1]:(("up-out",-0.5),("up",-1),("down-out",0.5),("down",1)),
                        self.dict_sub["01"]["lower_control"][0]:(("up-out",-0.5),("up",-1),("down-out",0.5),("down",1))
                        }
        ...
        def connect_sub_joint():
            list_locator = self.dict_sub["01"]["upper_joint"]+self.dict_sub["01"]["lower_joint"]

            for i,control in enumerate(self.dict_sub["01"]["upper_control"]+self.dict_sub["01"]["lower_control"]):
        ...
        def bind_skin_curve():
            ...
            # Name of the skinCluster and curve
            curve_shape = pm.listRelatives(self.curve_lip_main,c=1,s=1)[0]
            # Define the weights per CV
            weights = {
                'cv[0]': {
                    'lips_C0_L_main_jnt': 0.0,
                    'lips_C0_R_main_jnt': 1.0,
                    'lips_C0_low_main_jnt': 0.0,
                    'lips_C0_up_main_jnt': 0.0
                },
                'cv[1]': {
                    'lips_C0_L_main_jnt': 0.0,
                    'lips_C0_R_main_jnt': 0.2,
                    'lips_C0_low_main_jnt': 0.0,
                    'lips_C0_up_main_jnt': 0.8
                },
                'cv[2]': {
                    'lips_C0_L_main_jnt': 0.0,
                    'lips_C0_R_main_jnt': 0.0,
                    'lips_C0_low_main_jnt': 0.0,
                    'lips_C0_up_main_jnt': 1.0
                },
                'cv[3]': {
                    'lips_C0_L_main_jnt': 0.0,
                    'lips_C0_R_main_jnt': 0.0,
                ...
                'cv[5]': {
                    'lips_C0_L_main_jnt': 0.2,
                    'lips_C0_R_main_jnt': 0.0,
                    'lips_C0_low_main_jnt': 0.0,
                    'lips_C0_up_main_jnt': 0.8
                },
                'cv[6]': {
                    'lips_C0_L_main_jnt': 1.0,
                    'lips_C0_R_main_jnt': 0.0,
                    'lips_C0_low_main_jnt': 0.0,
                    'lips_C0_up_main_jnt': 0.0
                },
                'cv[7]': {
                    'lips_C0_L_main_jnt': 0.2,
                    'lips_C0_R_main_jnt': 0.0,
                    'lips_C0_low_main_jnt': 0.8,
                    'lips_C0_up_main_jnt': 0.0
                },
                'cv[8]': {
                    'lips_C0_L_main_jnt': 0.0,
                    'lips_C0_R_main_jnt': 0.0,
                    'lips_C0_low_main_jnt': 1.0,
                    'lips_C0_up_main_jnt': 0.0
                },
                'cv[9]': {
                    'lips_C0_L_main_jnt': 0.0,
        ...
        def main_control_to_main_joint():
        ...
        def create_scale_big_attr():
            list_name = self.dict_sub["01"]["upper_name"]+self.dict_sub["01"]["lower_name"]
            list_piv_m = self.dict_piv["upper_matrix"]+self.dict_piv["lower_matrix"]
            list_control = self.dict_sub["01"]["upper_control"]+self.dict_sub["01"]["lower_control"]
            dict_return = {}

            for i,loc in enumerate(self.dict_sub["01"]["upper_joint"]+self.dict_sub["01"]["lower_joint"]):
                name = list_name[i]
                ...
                if name in self.dict_sub["01"]["upper_name"]:
                    ...
                    elif i == len(self.dict_sub["01"]["upper_control"]) - 1:
        ...
        main_control_to_main_joint()

        create_pinch_control()

        create_sliding_locator()
