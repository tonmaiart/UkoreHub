...
from interface.shared.dialogs import AddonSettingsDialog
...
class AddonSettingsPage(QWidget):
    ...
    def _save_icon(self, addon_id: str, source_path) -> None:
        filename = save_image_asset(
            self, source_path=source_path, dest_dir=self.addon_store.icons_dir, asset_id=addon_id
        )
        if filename is not None:
            self.addon_store.set_icon(addon_id, filename)
