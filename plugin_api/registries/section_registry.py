...
class SectionSpec:
    ...
    # for this section. A section without either falls back to text-only
    # (e.g. a plugin that hasn't supplied an icon yet). icon_path is for a
    # real bundled bitmap asset (none of the built-in sections use this
    # anymore as of the zero-QSS/built-in-icons migration — kept available
    # for a future plugin that genuinely needs a custom bitmap, e.g. a
    # brand logo); standard_icon draws from the running Qt style's own
    # icon set instead (QStyle.standardIcon) and is what every built-in
    # section uses now. icon_path wins if both are set.
...
class SectionRegistry(KeyedOrderedRegistry[SectionSpec]):
    ...
    (plugin_api/registries/registry_base.py) — keys() specifically is used
    by launcher.py to diff before/after a single plugin's register(api)
    call and learn which section(s) that plugin contributed, for per-repo
    Plugin gating."""
