...
def _spec(key, order=0, **kwargs):
    return ProjectInfoTabSpec(key=key, label=key, order=order, page_factory=lambda: None, **kwargs)
...
def test_page_factory_is_stored_not_invoked():
    calls = []
    spec = ProjectInfoTabSpec(key="k", label="K", order=0, page_factory=lambda: calls.append("called"))
    registry = ProjectInfoTabRegistry()
